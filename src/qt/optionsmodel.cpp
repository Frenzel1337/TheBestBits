...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeTheBestBitsAmount"))
        settings.setValue("nAnonymizeTheBestBitsAmount", 1000);
    nAnonymizeTheBestBitsAmount = settings.value("nAnonymizeTheBestBitsAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeTheBestBitsAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeTheBestBitsAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeTheBestBitsAmount:
            return QVariant(nAnonymizeTheBestBitsAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeTheBestBitsAmount:
            nAnonymizeTheBestBitsAmount = value.toInt();
            settings.setValue("nAnonymizeTheBestBitsAmount", nAnonymizeTheBestBitsAmount);
            emit AnonymizeTheBestBitsAmountChanged(nAnonymizeTheBestBitsAmount);
            break;
        default:
            break;
        }
