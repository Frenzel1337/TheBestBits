...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("TheBestBits"))
        return false;
...
{
    // Convert TheBestBits:// to TheBestBits:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("TheBestBits://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "TheBestBits:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "TheBestBits.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for TheBestBits.lnk
...
{
    return GetAutostartDir() / "TheBestBits.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a TheBestBits.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=TheBestBits\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find TheBestBits app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add TheBestBits app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("TheBestBits-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  TheBestBits-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("TheBestBits-Qt"));
