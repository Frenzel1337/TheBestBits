...
int nAnonymizeTheBestBitsAmount = 1000;
...
#else
    const char* pszModule = "TheBestBits";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\TheBestBits
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\TheBestBits
    // Mac: ~/Library/Application Support/TheBestBits
    // Unix: ~/.TheBestBits
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "TheBestBits";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "TheBestBits.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "TheBestBitsd.pid"));
