...
void Shutdown()
{
    ...
    RenameThread("TheBestBits-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: TheBestBits.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: TheBestBitsd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeTheBestBitsamount=<n> " + _("Keep N TheBestBits anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. TheBestBits is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. TheBestBits is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("TheBestBits version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "TheBestBits server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of TheBestBits") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart TheBestBits to complete") << "\n";
    ...
    nAnonymizeTheBestBitsAmount = GetArg("-anonymizeTheBestBitsamount", 0);
    if(nAnonymizeTheBestBitsAmount > 999999) nAnonymizeTheBestBitsAmount = 999999;
    if(nAnonymizeTheBestBitsAmount < 2) nAnonymizeTheBestBitsAmount = 2;
    ...
    LogPrintf("Anonymize TheBestBits Amount %d\n", nAnonymizeTheBestBitsAmount);
